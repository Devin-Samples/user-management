"""Synchronous GitHub REST API client for fetching teams, members, and repos."""
...
class GitHubClient:
    ...
    def list_org_teams(self, org: str) -> list[GitHubTeam]:
        """Return all teams in a GitHub organization."""
    ...
    def list_team_repos(self, org: str, team_slug: str) -> list[GitHubRepo]:
        ...
        logger.info(
            "Fetched %d repos from GitHub team %s/%s",
            len(repos), org, team_slug,
        )
        return repos
    ...
    def get_saml_identities(self, org: str) -> dict[str, str]:
        ...
        Returns ``{github_login: saml_name_id}``.
        Requires admin:org scope and SSO configured on the org.
        """
        query = """
        ...
        while True:
            ...
            edges = (
                provider.get("externalIdentities", {}).get("edges", [])
            )
            for edge in edges:
                node = edge.get("node", {})
                user = node.get("user")
        ...
        logger.info(
            "Resolved %d SAML identities for org %s", len(result), org,
        )
        return result

    def get_audit_log_invite_emails(self, org: str) -> dict[str, str]:
        """Fetch invite emails from the org audit log.

        Returns ``{github_login: invitee_email}``.
        """
        ...
        try:
            for page_data in self._paginate(url, params=params):
                for entry in page_data:
                    login = entry.get("user", "").lower()
                    email = (
                        entry.get("data", {}).get("invitee_email")
                        or entry.get("invitee_email")
                    )
                    if login and email:
                        result[login] = email
        except Exception as exc:
            logger.warning(
                "Audit log query failed for org %s: %s", org, exc,
            )
    ...
    def _paginate(
        ...
        """Yield pages of JSON results following GitHub Link headers."""
        if params is None:
            params = {}
